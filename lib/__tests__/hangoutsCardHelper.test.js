...
describe('Rendering', () => {
  ...
  it('Text Card', () => {
    let textCard = createTextCard('This is the text');
    assert.deepEqual({ text: 'This is the text' }, textCard);
  });
  it('Compose card with cards and text', () => {
    let textCard = createTextCard('This is the text');
    let cardCard = singleImageCard('https://github.com/');
    let card = composeCard(textCard, cardCard);
    let { cards } = singleImageCard('https://github.com/');
    let { text } = createTextCard('This is the text');

    assert.deepEqual({ text, cards }, card);
  });
  it('Parameters creation', () => {
    let oneParam = createParamaterObject('numbers', '1234');
    let parameters = createParameterArray(oneParam, oneParam, oneParam);
    let expectedParam = { key: 'numbers', value: '1234' };
